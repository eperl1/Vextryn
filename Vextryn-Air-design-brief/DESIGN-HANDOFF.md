# b6bd5188-64d0-4f06-b74a-e9732f4e4b5c implementation handoff

This archive is the source of truth for turning the design into production code. Start from `vextryn-desktop-shell.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `vextryn-desktop-shell.html`
...
- A single self-contained `vextryn-desktop-shell.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `vextryn-desktop-shell.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
## Entry points
- `vextryn-desktop-shell.html`
- `vextryn-settings-app.html`
...
1. Inspect `vextryn-desktop-shell.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
