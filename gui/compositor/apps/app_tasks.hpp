...
static void draw_app_tasks(VxWindow& w, uint64_t frame, int mouse_x, int mouse_y, bool clicked) {
    ...
    vxr_fill_rect(ax, ay + 18, 40, 2, accent);
    ...
    // Divider
    vxr_fill_rect(ax, ay + 26, aw, 1, VxTheme::BORDER_SUBTLE);
    ...
    for (int i = 0; i < 12; i++) {
        ...
        // Row background
        if (hover) vxr_fill_rect(ax, iy, aw, item_h - 4, VxTheme::SURFACE_HIGH);
        ...
        vxr_fill_rect(cb_x, cb_y, 18, 18, cb_bg);
        vxr_fill_rect(cb_x, cb_y, 18, 1, cb_border);
        vxr_fill_rect(cb_x, cb_y + 17, 18, 1, cb_border);
        vxr_fill_rect(cb_x, cb_y, 1, 18, cb_border);
        vxr_fill_rect(cb_x + 17, cb_y, 1, 18, cb_border);

        // Checkmark
        if (g_tasks[i].done) {
            for (int j = 0; j < 6; j++) {
                vxr_fill_rect(cb_x + 3 + j, cb_y + 9 + j / 2, 2, 2, VxTheme::TEXT_PRIMARY);
            }
            for (int j = 0; j < 8; j++) {
                vxr_fill_rect(cb_x + 7 + j, cb_y + 12 - j / 2, 2, 2, VxTheme::TEXT_PRIMARY);
            }
        }
        ...
        if (g_tasks[i].done) {
            vxr_fill_rect(cb_x + 28, cb_y + 9, 35 * 8, 1, VxTheme::TEXT_MUTED);
        }
    ...
    vxr_fill_rect(btn_x, btn_y, btn_w, 28, add_hover ? VxTheme::accent_soft() : VxTheme::SURFACE_HIGH);
    vxr_fill_rect(btn_x, btn_y, btn_w, 1, VxTheme::BORDER_BRIGHT);
    vxr_fill_rect(btn_x, btn_y + 27, btn_w, 1, VxTheme::BORDER_SUBTLE);
    vxr_fill_rect(btn_x, btn_y, 1, 28, VxTheme::BORDER_SUBTLE);
    vxr_fill_rect(btn_x + btn_w - 1, btn_y, 1, 28, VxTheme::BORDER_SUBTLE);
