...
static void draw_app_dashboard(VxWindow& w, uint64_t frame, int mouse_x, int mouse_y, bool clicked) {
    ...
    const char* title = "Welcome back, Ethan";
    vx_text::draw(w.x + 20, w.y + 54, 16, title, VxTheme::TEXT_PRIMARY, VxTheme::SURFACE_1);
    const char* sub = "A focused workspace for building, browsing, and creating.";
    ...
    draw_dashboard_tile(content_x, content_y, tile_w, 84, "WORKSPACE", "READY", accent);
    draw_dashboard_tile(content_x + tile_w + 14, content_y, tile_w, 84, "APPS", "20", VxTheme::CYAN);
    draw_dashboard_tile(content_x, content_y + 96, tile_w, 84, "BROWSER", "ONLINE", accent);
    draw_dashboard_tile(content_x + tile_w + 14, content_y + 96, tile_w, 84, "FOCUS", "NOVA", VxTheme::CYAN);
    ...
    const char* section = "Start something";
    vx_text::draw_bold(right_x + 14, content_y + 22, 13, section, VxTheme::FG, VxTheme::GLASS_TINT);

    int button_y = content_y + 38;
    draw_dashboard_action(right_x + 14, button_y + 0, left_w - 28, 30, "Browse the web", mouse_x, mouse_y, clicked, VX_APP_BROWSER);
    draw_dashboard_action(right_x + 14, button_y + 38, left_w - 28, 30, "Open Files", mouse_x, mouse_y, clicked, VX_APP_FILES);
    draw_dashboard_action(right_x + 14, button_y + 76, left_w - 28, 30, "Open Studio", mouse_x, mouse_y, clicked, VX_APP_STUDIO);
    draw_dashboard_action(right_x + 14, button_y + 114, left_w - 28, 30, "Open Terminal", mouse_x, mouse_y, clicked, VX_APP_TERMINAL);
    ...
    const char* log_title = "Workspace notes";
    vx_text::draw_bold(content_x + 14, bottom_y + 22, 13, log_title, VxTheme::FG, VxTheme::GLASS_TINT);
    const char* lines[3] = {
        "Nocturne shell loaded.",
        "System Monitor removed from the workspace.",
        "Open the browser or Studio to get started."
    };
    for (int l = 0; l < 3; l++) {
