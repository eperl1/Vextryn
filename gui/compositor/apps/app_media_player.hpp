...
static void draw_app_media_player(VxWindow& w, uint64_t frame, int mouse_x, int mouse_y, bool clicked) {
    ...
    for (int y = 0; y < art_size; y++) {
        ...
        vxr_fill_rect(art_x, art_y + y, art_size, 1, c);
    }
    // Album art border
    vxr_fill_rect(art_x - 1, art_y - 1, art_size + 2, 1, VxTheme::BORDER_BRIGHT);
    vxr_fill_rect(art_x - 1, art_y + art_size, art_size + 2, 1, VxTheme::BORDER_STRONG);
    vxr_fill_rect(art_x - 1, art_y, 1, art_size, VxTheme::BORDER_STRONG);
    vxr_fill_rect(art_x + art_size, art_y, 1, art_size, VxTheme::BORDER_STRONG);

    // Music note icon centered on art
    int ncx = art_x + art_size / 2;
    int ncy = art_y + art_size / 2;
    vxr_fill_rect(ncx - 2, ncy - 16, 4, 20, VxTheme::TEXT_PRIMARY);
    vxr_fill_rect(ncx + 2, ncy - 16, 12, 4, VxTheme::TEXT_PRIMARY);
    vxr_fill_rect(ncx - 8, ncy + 2, 10, 8, VxTheme::TEXT_PRIMARY);
    ...
    int prog_x = ax + 20;
    int prog_w = aw - 40;
    vxr_fill_rect(prog_x, prog_y, prog_w, 6, VxTheme::BASE_DARK);
    vxr_fill_rect(prog_x, prog_y, prog_w, 1, VxTheme::BORDER_SUBTLE);
    int fill_w = (play_progress % 1200) * prog_w / 1200;
    if (fill_w > 0) vxr_fill_rect(prog_x, prog_y, fill_w, 6, accent);
    ...
    for (int dy = -20; dy <= 20; dy++) {
        ...
        if (half >= 0)
            vxr_fill_rect(play_x + 20 - half, play_y + 20 + dy, half * 2 + 1, 1, play_hover ? VxTheme::accent_glow() : accent);
    }
    if (is_playing) {
        // Pause icon
        vxr_fill_rect(play_x + 14, play_y + 12, 4, 16, VxTheme::TEXT_PRIMARY);
        vxr_fill_rect(play_x + 22, play_y + 12, 4, 16, VxTheme::TEXT_PRIMARY);
    } else {
        // Play icon (triangle)
        for (int i = 0; i < 12; i++)
            vxr_fill_rect(play_x + 14 + i, play_y + 10 + i, 2, 20 - i * 2, VxTheme::TEXT_PRIMARY);
    }
    ...
    // Prev track (|<)
    for (int i = 0; i < 8; i++) vxr_fill_rect(prev_t_x + 10 + i, ctrl_y + 10 + i, 2, 16 - i * 2, pt_hover ? accent : VxTheme::TEXT_SECONDARY);
    vxr_fill_rect(prev_t_x + 8, ctrl_y + 10, 2, 16, pt_hover ? accent : VxTheme::TEXT_SECONDARY);
    // Next track (>|)
    for (int i = 0; i < 8; i++) vxr_fill_rect(next_t_x + 18 - i, ctrl_y + 10 + i, 2, 16 - i * 2, nt_hover ? accent : VxTheme::TEXT_SECONDARY);
    vxr_fill_rect(next_t_x + 26, ctrl_y + 10, 2, 16, nt_hover ? accent : VxTheme::TEXT_SECONDARY);
